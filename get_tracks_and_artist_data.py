...
class SpotifyTrackArtistScraper(SpotifyIdScraper):
    def __init__(self, client_id, client_secret, filepath, writepath):
        super().__init__(client_id, client_secret, filepath, writepath)

    def get_tracks_and_artist_data(self):
        albums_found = pd.read_csv(self.filepath)
        grouped = albums_found.groupby('pitchfork_id')
        # albums.pitchfork_id.unique()
        tracks = []
        artists = []
        num_track = 0
        num_artist = 0
        for group in grouped:
            # mini_df = albums_found[albums_found.pitchfork_id == currId]
